...
/**
 * @namespace CityArtWalks.Components.Path.PathTableFiltersResult
 * @version 1.1.0
 * @author jaimie garner
 */

/**
 * Path Table Filters Result component for displaying active filters dynamically
 *
 ...
 * @param {Object} props.filterOptions - Filter options with geographic data
 ...
 */
export function PathTableFiltersResult({
  filters = {},
  totalResults,
  onResetPage,
  onClearFilters,
  ...
  const getFilterValueDisplay = useCallback(
    (value, filterKey) => {
      try {
        ...
        // Handle Path type enum
        if (filterKey === 'pathType') {
          const pathTypeDisplayNames = {
            WALKING: 'Walking',
            CYCLING: 'Cycling',
            DRIVING: 'Driving',
            PUBLIC_TRANSPORT: 'Public Transport',
          };
          return pathTypeDisplayNames[value] || value;
        }
        ...
        // Handle distance formatting (assuming meters)
        if (filterKey === 'distance') {
          const distanceKm = parseFloat(value) / 1000;
          return `${distanceKm.toFixed(1)} km`;
        }

        // Handle duration formatting (assuming minutes)
