...
import { RestartIcon, InfoCircleIcon } from 'src/components/icons';
      ...
      <LargeLabel>
        {canReset && (
          <ButtonBase disableRipple onClick={onReset} sx={{ ml: -0.5, mr: 0.5 }}>
            <RestartIcon size={14} sx={{ opacity: 0.64 }} />
          </ButtonBase>
        )}
        {title}
        {tooltip && (
          <Tooltip title={tooltip} placement="right" arrow>
            <InfoCircleIcon
              sx={{ ml: 0.5, mr: -0.5, opacity: 0.48, cursor: 'pointer', width: 14, height: 14 }}
            />
          </Tooltip>
        )}
      </LargeLabel>
