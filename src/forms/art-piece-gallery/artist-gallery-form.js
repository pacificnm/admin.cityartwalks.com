...
export default function ArtistGalleryForm({ currentImage, artistId, setCurrentTab }) {
  ...
  const ImageSchema = Yup.object().shape({
    ...
    imageUrl: Yup.mixed()
      .test('fileSize', 'File size is too large', (value) => !value || value.size <= 3145728)
  ...
  // Reset the form when the current image changes
  useEffect(() => {
    if (currentImage) {
      reset(defaultValues);
    }
      ...
      <Grid container spacing={3}>
        <Grid xs={12} md={4}>
          <Card sx={{ pt: 3, pb: 5, px: 3 }}>
            <Box sx={{ mb: 5 }}>
              <Typography variant="subtitle2">Artist Image</Typography>
              <RHFUpload
                name="imageUrl"
                maxSize={3145728}
                onDrop={handleDrop}
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentImage ? 'Create Image': 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
