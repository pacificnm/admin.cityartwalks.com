...
import { PostSort } from 'src/components/post/post-sort';
import { PostSearch } from 'src/components/post/post-search';
import { CustomBreadcrumbs } from 'src/components/custom-breadcrumbs';
import { PostListHorizontal } from 'src/components/post/post-list-horizontal';

// ----------------------------------------------------------------------

export function PostListView() {
  const { posts, postsLoading } = useGetPaginatedPosts();
...
function applyFilter({ inputData, filters, sortBy }) {
  ...
  if (publish !== 'all') {
    // Map the filter values to the actual status values from the API
    const statusMap = {
      published: 'PUBLISHED',
      draft: 'DRAFT',
    };
    const status = statusMap[publish];
    if (status) {
