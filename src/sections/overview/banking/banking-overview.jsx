...
import { AddIcon, ArrowUpIcon, InfoCircleIcon } from 'src/components/icons';
...
export function BankingOverview({ sx, ...other }) {
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      >
        ...
        <Tooltip title="Vestibulum ullamcorper mauris">
          <InfoCircleIcon sx={{ color: 'text.disabled', width: 16, height: 16 }} />
        </Tooltip>
      </Box>
  ...
  const renderActions = () => (
    <Box sx={{ gap: 1, display: 'flex' }}>
      <Button variant="soft" size="small" startIcon={<ArrowUpIcon size={16} />}>
        Send
      </Button>
      <Button variant="soft" size="small" startIcon={<AddIcon size={16} />}>
        Add card
      </Button>
      <Button
        variant="soft"
        size="small"
        startIcon={<Iconify width={16} icon="solar:arrow-down-bold" />}
      >
        Request
      </Button>
    </Box>
