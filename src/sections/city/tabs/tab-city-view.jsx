...
import { CityEditDialog } from 'src/components/city/city-edit-dialog';

export function TabCityView({ cities }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return cities.filter((city) => {
      const matchesKeyword = [city.name, city.slug].some((field) =>
  ...
  }, [cities, filters.name, filters.status]);
...
const columns = [
  ...
  { field: 'longitude', label: 'Longitude', render: (_, row) => row?.longitude || 'Unknown' },
  {
    field: 'active',
    label: 'Active',
    render: (value) => <Chip label={value ? 'Yes' : 'No'} color={value ? 'primary' : 'default'} />,
  },
];
