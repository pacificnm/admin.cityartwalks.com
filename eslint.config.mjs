...
// Note: eslint-config-next is not compatible with ESLint 9 flat config
...
const commonRules = () => ({
  ...
  // Next.js specific rules (manually added since eslint-config-next is not compatible with ESLint 9)
  'react/jsx-no-target-blank': 2,
  'react/no-unescaped-entities': 1,
  // Note: @next/next plugin rules would need to be added here if the plugin was available for flat config
});
...
export default [
  ...
  // Note: Next.js ESLint config is not compatible with ESLint 9 flat config
  // We include Next.js specific rules manually in customConfig
  customConfig,
];
